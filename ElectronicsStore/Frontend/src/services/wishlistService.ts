...
class WishlistService {
  ...
  async addMultipleItems(
    ...
    const successfulItems = responses
      ...
      .map(
        (response) =>
          (response as PromiseFulfilledResult<ApiResponse<WishlistItem>>).value
            .data!
      )
      .filter(Boolean);

    const failedItems = responses.filter(
      (response) => response.status === "rejected" || !response.value.success
    );
  ...
  async moveToCart(
    ...
    ApiResponse<{ removedFromWishlist: boolean; addedToCart: boolean }>
  > {
    try {
      ...
      // Remove from wishlist
      const removeFromWishlistResponse = await this.removeFromWishlist(
        productId
      );
  ...
  async moveAllToCart(): Promise<
    ApiResponse<{ movedItems: number; failedItems: number }>
  > {
    try {
  ...
  > {
    ...
    items.forEach((item) => {
      if (!item.product || !item.product.brand) return;
  ...
  // Price tracking
  async checkPriceChanges(): Promise<
    ApiResponse<
      Array<{
        productId: number;
        productName: string;
    ...
    try {
      ...
      for (const wishlistItem of wishlistResponse.data) {
        ...
        if (productResponse.success && productResponse.data) {
          ...
          const storedPrice = wishlistItem.product?.price ?? 0; // Price when added to wishlist
